...
import org.yankov.mso.application.ui.table.FontModel._
...
case class FolkloreTrackTable(inputTable: Boolean) {
  ...
  private val table = new TableView[FolkloreTrackProperties] {
    ...
    items.addListener(_ => handleTableChange())

    rowFactory = _ => new TableRow[FolkloreTrackProperties] {
      onMouseClicked = event =>
        if (event.getClickCount == 2) ApplicationContext.executeCommand(/*Commands.openFolkloreTrackEditor, table*/)
    }
  }
  ...
  private def createTableColumns: List[TableColumn[FolkloreTrackProperties, _]] = {
    val columns = List(
      ...
      new TableColumn[FolkloreTrackProperties, String] {
        ...
        cellValueFactory = {_.value.performer}
      },
      new TableColumn[FolkloreTrackProperties, String] {
        text = Resources.TableColumns.accompanimentPerformer
        cellValueFactory = {_.value.accompanimentPerformer}
      },
      new TableColumn[FolkloreTrackProperties, String] {
        text = Resources.TableColumns.arrangementAuthor
        cellValueFactory = {_.value.arrangementAuthor}
      },
      new TableColumn[FolkloreTrackProperties, String] {
        text = Resources.TableColumns.conductor
        cellValueFactory = {_.value.conductor}
      },
      new TableColumn[FolkloreTrackProperties, String] {
        text = Resources.TableColumns.author
        cellValueFactory = {_.value.author}
      },
      new TableColumn[FolkloreTrackProperties, String] {
        text = Resources.TableColumns.soloist
        cellValueFactory = {_.value.soloist}
      },
      new TableColumn[FolkloreTrackProperties, String] {
      ...
      new TableColumn[FolkloreTrackProperties, String] {
        ...
        cellValueFactory = {_.value.ethnographicRegion}
      },
      new TableColumn[FolkloreTrackProperties, String] {
        text = Resources.TableColumns.note
        cellValueFactory = {_.value.note}
      }
    )
  ...
  private val columnWidths: List[Double] =
    List(800, 600, 600, 400, 400, 200, 200, 200, 300, 300, 200, 200)
      .map(x => x.toDouble)

  private def handleTableChange(): Unit = resizeColumns()

  private def resizeColumns(): Unit = {
