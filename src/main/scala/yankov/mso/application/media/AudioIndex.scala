...
import yankov.mso.application.{FileUtils, Resources}
import yankov.mso.application.database.Database
import yankov.mso.application.media.decode.FlacDecoder
import yankov.mso.application.model.DataModel.{AudioInput, AudioSearchData, AudioSearchResult, AudioSearchSample, ExactMatch, SimilarMatch}
    ...
    if (a.hash.equals(b.hash)) {
      ...
      crossCorrelation(a.data, b.data, crossCorrelationShift) match {
        case Some(correlation) =>
          if (abs(correlation) > correlationThreshold) {
