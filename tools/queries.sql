connect 'jdbc:derby:folklore-media-storage-organizer/database';
disconnect;

describe ADMIN.FOLKLORE_TRACK;
describe ADMIN.RECORD;
...
SELECT COUNT(*) FROM ADMIN.RECORD;
SELECT COUNT(*) FROM ADMIN.RECORD WHERE "DATA" IS NOT NULL;
...
CALL SYSCS_UTIL.SYSCS_EXPORT_TABLE_LOBS_TO_EXTFILE('ADMIN', 'RECORD', 'record.txt', '#', '@', 'UTF-8', 'record.bin');

CALL SYSCS_UTIL.SYSCS_IMPORT_TABLE_LOBS_FROM_EXTFILE('ADMIN', 'FOLKLORE_TRACK', 'result.txt', '#', '@', 'UTF-8', 1);
